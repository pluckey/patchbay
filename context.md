...
- Glob `src/kernel/source-kinds/{index,registry,contribution}.ts` for the source-kind plugin registry (kernel side: pure registry + contribution type, no concrete kinds)
- Glob `src/client/source-kinds/{index,markdown-source-kind,pdf-source-kind,derived-source-kind}.ts` for the concrete source-kind contributions (client side: registered into the kernel registry on import)
- Glob `src/kernel/entities/workspace-ref.ts` for WorkspaceRef (multi-workspace registry metadata)
- Glob `src/client/domain/ports/*.ts` for port interfaces (StoragePort, BlobStoragePort, PdfRendererPort, TransformExecutorPort, ChatPort, ModelRosterPort, AiExecutorPort, WorkspaceRegistryPort, DeletionManifestPort — AiExecutorPort accepts optional schema, schemaMode, and maxTokens for single/collection structured output with a configurable output-token budget)
